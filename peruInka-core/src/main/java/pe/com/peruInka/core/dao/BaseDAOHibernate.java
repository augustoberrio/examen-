...
//	}
	...
	public void save(UserSystem userSystem) throws DataAccessException {
		Date systemDate = new Date();
		userSystem.setDateLastUpdated(systemDate);
		if (userSystem.getId() == null || userSystem.getDateCreated() == null) {
			userSystem.setDateCreated(systemDate);
			getHibernateTemplate().save(userSystem);
		}else{
			try {
				...
				if (systemExc.getRootCause() instanceof NonUniqueObjectException) {
					getHibernateTemplate().merge(userSystem);
				} else {
					logger.info("0%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
